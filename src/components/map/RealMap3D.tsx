...
export const RealMap3D: React.FC<RealMapProps> = ({
  ...
  useEffect(() => {
    ...
    locations.forEach((location) => {
      ...
      // Wrapper để Mapbox thêm translate(...)
      const wrapperEl = document.createElement("div");
      wrapperEl.className = "custom-marker-wrapper";
    
      // Inner (giữ nguyên CSS bạn yêu cầu)
      ...
      `;
      innerEl.innerHTML = location.type === "attraction" ? "🏞️" : "🏺";
    
      // Scale nếu đang được chọn
      ...
      const marker = new mapboxgl.Marker(wrapperEl)
        .setLngLat([lng, lat])
        .addTo(map.current!);
    
      // Popup tạo trước nhưng KHÔNG add ngay
      ...
      innerEl.addEventListener("mouseenter", () => {
        // scale nhẹ khi hover (nếu chưa selected)
        if (selectedLocation?.id !== location.id) {
      ...
      // --- Hỗ trợ cảm ứng (tap để toggle popup) ---
      let touchOpen = false;
      ...
      // Click/Touch ngoài để đóng popup cảm ứng
      document.addEventListener("touchstart", (ev) => {
        if (!wrapperEl.contains(ev.target as Node) && touchOpen) {
          popup.remove();
          touchOpen = false;
          innerEl.style.transform =
            selectedLocation?.id === location.id ? "scale(1.3)" : "scale(1)";
        ...
        <Card className="w-full max-w-md shadow-floating">
          ...
          <CardContent className="space-y-4">
            <div className="text-sm text-muted-foreground space-y-2">
              <p>
                🗺️ To show realistic 3D terrain of Quang Binh province, we need
                a Mapbox API key.
              </p>
              <p>
                📍 Get your <strong>free</strong> Mapbox token:
              </p>
            ...
            <div className="text-xs text-muted-foreground bg-muted/50 p-3 rounded-lg">
              <p>
                <strong>💡 Tip:</strong> Your token is stored locally in your
                browser. For production apps, use Supabase for secure secret
                management.
              </p>
            </div>
          </CardContent>
      ...
      {isMapReady && (
        <div className="absolute top-4 left-4 bg-card/90 backdrop-blur-sm rounded-lg p-3 shadow-nature">
          ...
          <p className="text-xs text-muted-foreground">
            Real 3D terrain • {locations.length} locations
          </p>
        </div>
      )}
    </div>
